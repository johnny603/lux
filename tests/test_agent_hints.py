...
def test_hint_generation_falls_back_between_models(monkeypatch):
    ...
    def fake_chat(model, messages):
        ...
        return SimpleNamespace(message=SimpleNamespace(content="Use a smaller step. Avoid revealing the answer directly. And check the tags."))
    ...
    hint = agent.ask_hint_via_ollama(
        {"id": "1", "title": "Test", "description": "Desc", "category": "Linux", "difficulty": "easy", "tags": ["shell"]},
...
def test_hint_generation_raises_when_all_models_fail(monkeypatch):
    ...
    try:
        agent.ask_hint_via_ollama(
            {"id": "1", "title": "Test", "description": "Desc", "category": "Linux", "difficulty": "easy", "tags": []},
            state=storage.default_state(),
        )
